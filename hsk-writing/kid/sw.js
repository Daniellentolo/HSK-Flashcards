/* HSK 1 Writing (Kids) service worker — scoped to /hsk-writing/kid/ with its own caches so it never touches the adult writing app */
const SHELL="hskkid-shell-v1";
const RUN="hskkid-runtime-v1";
const SHELL_ASSETS=[
...
self.addEventListener("activate",e=>{
  e.waitUntil(caches.keys().then(keys=>Promise.all(
    keys.filter(k=>k.startsWith("hskkid-")&&k!==SHELL&&k!==RUN).map(k=>caches.delete(k))
  )));
  self.clients.claim();
});
